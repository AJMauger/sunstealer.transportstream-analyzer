...
namespace ajm::iso13818_1 {
  ...
  void PAT::Debug() {
    ...
    for(std::vector<std::shared_ptr<Table::Entry>>::iterator i=this->entries.begin(); i!=this->entries.end(); i++) {
      PAT::Entry *entry=dynamic_cast<PAT::Entry *>((*i).get());
      std::printf("\n{");
  ...
  void PAT::OnSection() {
    ...
    while(stream<end) {
      this->entries.push_back(std::shared_ptr<PAT::Entry>(new PAT::Entry(this->name, &stream, this)));
    }
  ...
  PAT::Entry::Entry(std::string name, std::uint8_t **stream, PAT *table)
    : Table::Entry(name, table)  {
      ...
      SectionHeader1 *header=reinterpret_cast<SectionHeader1 *>(table->section->pid->transportStream->pids[pid]->section->payload);
