...
namespace ajm::iso13818_1 {
  ...
  void PMT::Debug() {
    ...
    for(std::vector<std::shared_ptr<Descriptor >>::iterator i=this->descriptors.begin(); i!=this->descriptors.end(); i++) {
      std::printf("\n  tag: %d 0x%02x", (*i)->stream[0], (*i)->name);
    }
    std::printf("\n  }");
    for(std::vector<std::shared_ptr<Table::Entry >>::iterator i=this->entries.begin(); i!=this->entries.end(); i++) {
      std::printf("\n  entry {");
      PMT::Entry *entry=dynamic_cast<PMT::Entry *>((*i).get());
      std::printf("\n    es_info_descriptors[%d]: {", entry->descriptors.size());
      for(std::vector<std::shared_ptr<Descriptor>>::iterator j=entry->descriptors.begin(); j!=entry->descriptors.end(); j++) {
  ...
  void PMT::OnSection() {
    ...
    std::uint8_t *end=stream+this->programInfoLength;
    while(stream<end) {
    ...
    while(stream<end) {
      this->entries.push_back(std::shared_ptr<PMT::Entry>(new PMT::Entry(this->name, &stream, this)));
    }
  ...
  PMT::Entry::Entry(std::string name, std::uint8_t **stream, Table *table)
    ...
    try {
      ...
      while(*stream<end) {
        this->descriptors.push_back(std::shared_ptr<Descriptor>(new Descriptor(stream, table)));
      }
    } catch(...) {
