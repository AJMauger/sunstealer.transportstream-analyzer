...
namespace ajm::iso13818_1 {
  ...
  void Section::OnPacket(Packet *packet) {
    try {
      ...
      PacketHeader *packetHeader=reinterpret_cast<PacketHeader *>(packet->payload);
      ...
      if(packetHeader->payload_start_indicator) {
        ...
        SectionHeader1 *sectionHeader=reinterpret_cast<SectionHeader1 *>(&packet->payload[188-payloadLength]);
    ...
    try {
      ...
      if(table) {
        ...
        if (j==table->sections.end()) { // ajm: new section
          std::uint8_t *payload=new std::uint8_t[this->length];
