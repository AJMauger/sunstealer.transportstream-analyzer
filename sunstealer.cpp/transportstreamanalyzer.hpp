...
namespace ajm::transportstreamanalyzer
{
  class Plugin {
    ...
    public: void (*destroy)();
    public: void *handle=0;

    public: ajm::framework::Device *device=0;
    public: ajm::iso13818_1::TransportStream *transportStream=0;
  };

  class TransportStreamAnalyzer : public ajm::framework::IDevice, public ajm::iso13818_1::TransportStream {
    // ajm: ---------------------------------------------------------------------------------------
    public: TransportStreamAnalyzer(ajm::framework::ILogger *logger) 
    ...
    public: void Analyze(std::string uri) {
      try {
        ...
        std::map<std::string, Plugin *>::iterator i=this->plugins.find("iso-13818-1::service-information");
    ...
    public: void OnState(ajm::framework::IDevice::eSTATE state) {
      switch(state) {
        ...
        case ajm::framework::IDevice::eSTATE::eSTATE_STOP:
          for(std::map<std::string, Plugin *>::iterator i=this->plugins.begin(); i!=plugins.end(); i++) {
            if ((*i).second!=this->plugin) {
      ...
      try {
        ...
        if (handle) {
          ...
          if (getname) {
            ...
            void (*destroy)() = (void (*)()) dlsym(handle, "Destroy");
    ...
    void UnloadPlugins() {
      try {
        ...
        for (std::map<std::string, Plugin *>::iterator i=this->plugins.begin(); i!=this->plugins.end(); i++) {
          (*i).second->destroy();
    ...
    public: ajm::framework::ILogger *logger=0;
    public: Plugin *plugin=0;
    public: std::map<std::string, Plugin *> plugins;
  };
}