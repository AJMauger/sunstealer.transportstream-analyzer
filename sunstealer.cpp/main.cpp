...
// ajm: #include "../sunstealer.logging/event.hpp"
// ajm: #include "../sunstealer.logging/logger.hpp"
...
int main(int argc, char** argv)
{
  try {
    ...
    if(argc % 2 != 1) {
      std::printf("Usage: ./sunstealer [-c client | server].");
      return -1;
    }

    for (int i=1; i<argc; i+=2) {
      options.insert(std::pair(argv[i], argv[i+1]));
    }
    ...
    // ajm::logging::so::Logger *logger = new ajm::logging::so::Logger(10000, "/home/ajm/Documents/sunstealer/Logs", 500, "logger");
    ...
    std::printf("DEBUG: main std::this_thread::id: %" PRIu64 ".\n", std::this_thread::get_id());
    ...
    if (handle)
    {
      ...
      ajm::framework::ILogger *(*destroy)(const char *name) = (ajm::framework::ILogger * (*)(const char *name)) dlsym(handle, "Destroy");
      ...
      if (create && destroy)
      {
        ...
        if (ajm::system::System::_system) {
          ...
          if (command == "transportstreamanalyzer") {
            logger->LogDebug(std::format("Main(): command: %s", command.c_str()).c_str());
